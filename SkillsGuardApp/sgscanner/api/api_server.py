...
class InspectRequest(BaseModel):
    ...
    use_dataflow: bool = Field(False, description='Enable dataflow detector (behavioral analysis)')
    use_dataflow: bool = Field(False, description='Enable dataflow detector (preferred)')
    use_aidefense: bool = Field(False, description='Enable SkillsGuard AI Defense detector')
    aidefense_api_key: str | None = Field(None, description='AI Defense API key (or use AI_DEFENSE_API_KEY env var)')
    enable_meta: bool = Field(False, description='Enable meta detector to filter false positives and prioritize findings')
    ...
    try:
        ...
        if request.enable_meta and META_AVAILABLE and (len(result.findings) > 0):
            try:
                ...
                meta_result = await loop.run_in_executor(executor, lambda: async_lib.run(meta_detector.analyze_with_findings(skill=skill, findings=result.findings, engines_used=result.engines_used)))
...
async def scan_batch(request: BatchInspectRequest, background_tasks: BackgroundTasks):
    ...
    background_tasks.add_task(run_batch_scan, scan_id, skills_dir, request.recursive, request.use_semantic, request.use_semantic, request.llm_provider, request.use_dataflow, request.use_dataflow, request.use_aidefense, request.aidefense_api_key, request.enable_meta)
    return {'scan_id': scan_id, 'status': 'processing', 'message': 'Batch scan started. Use GET /inspect-batch/{scan_id} to check status.'}

@app.get('/inspect-batch/{scan_id}')
...
def run_batch_scan(scan_id: str, skills_dir: Path, recursive: bool, use_semantic: bool, use_semantic: bool, llm_provider: str | None, use_dataflow: bool=False, use_dataflow: bool=False, use_aidefense: bool=False, aidefense_api_key: str | None=None, enable_meta: bool=False):
    try:
        import os
        from ..engines.base import ScanEngine
        detectors: list[ScanEngine] = [PatternEngine()]
                ...
                for result in report.scan_results:
                    if result.findings:
                        try:
                            ...
                            skill = scanner._ingester.ingest(skill_dir_path)
...
def run_server(host: str='localhost', port: int=8000, reload: bool=False):
    import uvicorn
    uvicorn.run('sgscanner.api.api_server:app', host=host, port=port, reload=reload)
