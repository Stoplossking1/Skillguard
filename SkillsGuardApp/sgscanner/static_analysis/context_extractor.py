...
class SkillScriptContext:
    ...
    def serialize(self) -> dict[str, Any]:
        return {'file_path': self.file_path, 'function_count': len(self.functions), 'imports': self.imports, 'security_indicators': {'has_network': self.has_network, 'has_file_ops': self.has_file_ops, 'has_subprocess': self.has_subprocess, 'has_eval_exec': self.has_eval_exec, 'has_credential_access': self.has_credential_access, 'has_env_var_access': self.has_env_var_access}, 'dangerous_patterns': {'exfiltration_chain': self.has_exfiltration_chain, 'injection_chain': self.has_injection_chain, 'dangerous_flow_count': len(self.dangerous_flows)}, 'functions': [{'name': f.name, 'parameters': f.parameters, 'has_network': f.has_network_calls, 'has_file_ops': f.has_file_operations, 'has_subprocess': f.has_subprocess, 'has_eval_exec': f.has_eval_exec, 'calls': f.function_calls[:10]} for f in self.functions], 'suspicious_urls': self.suspicious_urls}
...
class ContextExtractor:
    SUSPICIOUS_DOMAINS = ['pastebin.com', 'hastebin.com', 'paste.ee', 'rentry.co', 'zerobin.net', 'textbin.net', 'termbin.com', 'sprunge.us', 'clbin.com', 'ix.io', 'pastetext.net', 'pastie.org', 'ideone.com', 'transfer.sh', 'filebin.net', 'gofile.io', 'anonfiles.com', 'mediafire.com', 'mega.nz', 'wetransfer.com', 'filetransfer.io', 'ufile.io', '4sync.com', 'uplooder.net', 'filecloudonline.com', 'sendspace.com', 'siasky.net', 'webhook.site', 'requestbin', 'ngrok.io', 'pipedream.net', 'localhost.run', 'trycloudflare.com', 'codepen.io', 'repl.co', 'glitch.me', 'attacker.example.com', 'evil.example.com', 'malicious.com', 'c2-server.com']
    LEGITIMATE_DOMAINS = ['api.anthropic.com', 'statsig.anthropic.com', 'api.openai.com', 'api.together.xyz', 'api.cohere.ai', 'generativelanguage.googleapis.com', 'registry.npmjs.org', 'npmjs.com', 'npmjs.org', 'yarnpkg.com', 'registry.yarnpkg.com', 'pypi.org', 'files.pythonhosted.org', 'pythonhosted.org', 'crates.io', 'rubygems.org', 'pkg.go.dev', 'archive.ubuntu.com', 'security.ubuntu.com', 'debian.org', 'schemas.microsoft.com', 'schemas.openxmlformats.org', 'www.w3.org', 'purl.org', 'json-schema.org', 'localhost', '127.0.0.1', '0.0.0.0', '::1', 'stripe.com', 'zoom.us', 'twilio.com', 'mailgun.com', 'sentry.io', 'datadog.com', 'newrelic.com', 'elastic.co', 'mongodb.com', 'redis.io', 'postgresql.org']
    ...
    def extract_context(self, file_path: Path, source_code: str) -> SkillScriptContext:
        ...
        try:
            ...
            import logging
            logging.getLogger('sg.' + __name__).warning(f'CFG-based script-level analysis failed: {e}')
            script_flows = []
        ...
        for flow in script_flows:
            if flow.reaches_external:
                ...
                if any((any((nc in call.lower() for nc in network_calls)) for call in flow.reaches_calls)):
                    ...
                    sink_type = 'eval'
                dangerous_flows.append({'source_type': source_type, 'source_name': flow.parameter_name, 'sink_type': sink_type, 'sink_operation': ', '.join(flow.reaches_calls), 'is_dangerous': True})
        has_exfiltration_chain = any((flow.get('source_type') in ['credential_file', 'env_var'] and flow.get('sink_type') == 'network' for flow in dangerous_flows))
        has_injection_chain = any((flow.get('source_type') == 'parameter' and flow.get('sink_type') == 'eval' for flow in dangerous_flows))
        all_calls = []
        all_strings = []
        ...
        context = SkillScriptContext(file_path=str(file_path), functions=parser.functions, imports=parser.imports, dataflows=[], has_network=has_network, has_file_ops=has_file_ops, has_subprocess=has_subprocess, has_eval_exec=has_eval_exec, has_credential_access=has_credential_access, has_env_var_access=has_env_var_access, dangerous_flows=dangerous_flows, has_exfiltration_chain=has_exfiltration_chain, has_injection_chain=has_injection_chain, all_function_calls=list(set(all_calls)), all_string_literals=all_strings, suspicious_urls=suspicious_urls)
        return context
    ...
    def _extract_function_context(self, node: ast.FunctionDef, imports: list[str], source_code: str, file_path: Path) -> SkillFunctionContext:
        name = node.name
        ...
        return SkillFunctionContext(name=name, docstring=docstring, parameters=parameters, return_type=return_type, line_number=line_number, imports=imports, function_calls=function_calls, assignments=assignments, control_flow=control_flow, parameter_flows=parameter_flows, constants=constants, variable_dependencies=var_deps, has_file_operations=has_file_ops, has_network_operations=has_network_ops, has_subprocess_calls=has_subprocess, has_eval_exec=has_eval_exec, string_literals=string_literals, return_expressions=return_expressions, exception_handlers=exception_handlers, env_var_access=env_var_access, global_writes=global_writes, attribute_access=attribute_access, dataflow_summary=dataflow_summary)
    ...
    def _extract_function_calls(self, node: ast.FunctionDef) -> list[dict[str, Any]]:
        ...
        for child in ast.walk(node):
            if isinstance(child, ast.Call):
                ...
                call_info = {'name': self._get_call_name(child), 'args': args_list, 'line': child.lineno if hasattr(child, 'lineno') else 0}
                ...
                for target in child.targets:
                    if isinstance(target, ast.Name):
                        try:
                            ...
                            value_str = '<complex>'
                        assignments.append({'variable': target.id, 'value': value_str, 'line': child.lineno if hasattr(child, 'lineno') else 0})
        return assignments
    ...
    def _analyze_control_flow(self, node: ast.FunctionDef) -> dict[str, Any]:
        ...
        return {'has_conditionals': has_if, 'has_loops': has_for or has_while, 'has_exception_handling': has_try}
    ...
    def _analyze_parameter_flows(self, node: ast.FunctionDef, parameters: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        try:
            ...
            for flow_path in flow_paths:
                flows.append({'parameter': flow_path.parameter_name, 'operations': flow_path.operations, 'reaches_calls': flow_path.reaches_calls, 'reaches_assignments': flow_path.reaches_assignments, 'reaches_returns': flow_path.reaches_returns, 'reaches_external': flow_path.reaches_external})
        except Exception as e:
            import logging
            logging.getLogger('sg.' + __name__).warning(f'CFG-based parameter flow analysis failed: {e}')
            return flows
        return flows
    ...
    def _extract_exception_handlers(self, node: ast.FunctionDef) -> list[dict[str, Any]]:
        ...
        for child in ast.walk(node):
            if isinstance(child, ast.ExceptHandler):
                handler_info = {'line': child.lineno, 'exception_type': ast.unparse(child.type) if child.type else 'Exception', 'is_silent': len(child.body) == 1 and isinstance(child.body[0], ast.Pass)}
                ...
                for target in child.targets:
                    if isinstance(target, ast.Attribute):
                        ...
                        try:
                            ...
                            value_str = '<complex>'
                        attribute_ops.append({'type': 'write', 'object': obj_name, 'attribute': target.attr, 'value': value_str, 'line': child.lineno})
        return attribute_ops[:20]

    def _create_dataflow_summary(self, node: ast.FunctionDef) -> dict[str, Any]:
        return {'total_statements': len([n for n in ast.walk(node) if isinstance(n, ast.stmt)]), 'total_expressions': len([n for n in ast.walk(node) if isinstance(n, ast.expr)]), 'complexity': self._calculate_complexity(node)}
